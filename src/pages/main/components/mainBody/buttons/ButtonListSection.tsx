...
import { createNewBlankButton, type Tag } from "@/store/button-sets/buttonSetSlice.ts";
...
const ButtonListSection = ({ tagId = -1 }: Props) => {
    ...
    return (
        <div className={"w-fit flex flex-col gap-4 my-4"}>
            <div className={"flex flex-row gap-8 justify-between border-b-4 w-fit p-2"}>
                <h1 className={"text-4xl font-bold text-left text-wrap w-fit"}>{formattedTagName}</h1>
