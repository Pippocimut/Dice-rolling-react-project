...
export function CustomRollToast({ historyData }: Props) {
    ...
    return (<div className="w-fit flex font-bold text-lg flex-col items-start justify-start p-2 font-boldreact-router-dom">
        ...
        {onlyOneRoll && historyData.rollResult[0].type == "roll" && <div className={"text-left py-2 pl-2"}>
        ...
        {onlyOneRoll && historyData.rollResult[0].type == "text" && <div className={"text-left py-2 pl-2"}>
            <div className={"flex flex-col"}>
                <p>
                    {historyData.rollResult[0].text}
                </p>
            </div>
        </div>}
        {moreThanOneRoll && historyData.rollResult.map((rollResult, index) => {
            if (rollResult.type === "text") {
                return (
