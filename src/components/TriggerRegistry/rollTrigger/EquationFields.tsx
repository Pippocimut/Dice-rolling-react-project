...
export const EquationFields = ({ currentEquation, sideEffect }: Props) => {
    const trigger = useSelector((state: RootState) => state.buttonManage.trigger)
    if (trigger.type !== "roll") return null

    const dispatch = useDispatch()
    const updateRoll = (roll: RollTrigger) => dispatch(setRoll(roll))
    ...
    const deleteSideEffect = (equationId: number) => (sideEffectId: number) => () => {
        ...
        updateRoll({
            ...trigger,
            equations: { ...trigger.equations, [equationId]: newEquation }
        })
    }
        ...
        >
            <div className="flex flex-row gap-4 items-center">
                ...
                <Button
                    ...
                    variant="empty"
                    onClick={deleteSideEffect(currentEquation.id)(sideEffect.id)}
                >X</Button>
            </div>
...
const EquationTriggerSelection = ({ equation, sideEffectId }: EquationTriggerSelectionProps) => {
    ...
    const currentTrigger = useSelector((state: RootState) => {
        ...
        return state.buttonSet.sets[path[0].id]?.buttonList[path[1].id]?.triggers[path[2].id];
    })
...
const TriggerList = ({ triggers }: { triggers: Trigger[] }) => {
    ...
    return <>
        {triggers.map((t) => {
            if (t.id === 0 || t.name === "" || t.id === roll.id) return null;
            return (
